  ...
  const verifyCredentials = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const website = await websiteService.findOrCreate(fullDomain, userInfo.name || 'WordPress Site')
        ...
        throw new Error(`HTTP ${response.status}: ${errorText || 'Invalid credentials or API access denied'}`)
      }
    } catch (error: any) {
...
<script type="application/ld+json">
{
  ...
  "sameAs": [
    ...
    "https://linkedin.com/company/yourcompany"
      ...
      const response = await fetch(`${domain}/wp-json/wp/v2/posts`, {
        ...
        body: JSON.stringify({
          title: `SEO Schema Update - ${new Date().toLocaleDateString()}`,
          content: `<p>This post contains updated JSON-LD schema markup for improved SEO.</p>${schemaMarkup}`,
          status: 'publish',
          meta: {
  ...
  const deleteIntegration = async (integration: IWordPressIntegration) => {
    if (!confirm('Are you sure you want to delete this WordPress integration?')) return
              ...
              >
                <div className="flex-1">
                  ...
                  <div className="text-sm text-gray-600">
                    ...
                    Status: <span className={`font-medium ${
                      integration.connection_status === 'connected' ? 'text-green-600' : 'text-red-600'
                    }`}>
                ...
                <div className="flex items-center space-x-2">
                  <div className={`w-3 h-3 rounded-full ${
                    integration.connection_status === 'connected' ? 'bg-green-500' : 'bg-red-500'
                  }`} />
                  <button
                    onClick={(e) => {
                      e.stopPropagation()
