...
export const useTracking = () => {
    ...
    (eventName: string, properties?: Record<string, any>) => {
      try {
        if ((window as any).track) {
          (window as any).track("customEvent", eventName, properties);
        }
      } catch (error) {
  ...
  const trackSchemaAudit = useCallback(
    (url: string) => {
  ...
  const trackPublishSchema = useCallback(
    (domain: string, postId?: string) => {
  ...
  const trackConnectWordPress = useCallback(
    (domain: string) => {
  ...
  const trackPostAudit = useCallback(
